...
```
freecell/
├── game.py           # 规则逻辑（纯 Python，不依赖 pygame，可单测）
├── main.py           # pygame 界面与交互（渲染、拖拽、按钮、快捷键）
├── test_game.py      # 基础规则单元测试
├── test_stress.py    # 边界与压力测试（随机对局、撤销对称性、UI 交互）
├── requirements.txt  # 依赖（pygame-ce）
├── README.md         # 本文档
├── preview_normal.png  # 正常对局预览图
└── preview_win.png     # 胜利弹窗预览图
```
